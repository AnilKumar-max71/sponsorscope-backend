  ...
  try {
    ...
    if (!allowedDomains.includes(domain)) {
      return res.status(400).json({ 
        error: 'Please use Gmail, Outlook, Yahoo, or Hotmail email address' 
      });
    }
      ...
      .insert([
        {
          ...
          instance_id: crypto.randomUUID(),    // ADDED: Required by Supabase
...
app.post('/api/auth/verify-email', authLimiter, async (req, res) => {
  try {
    ...
    // Mark email as verified - UPDATED FOR SCHEMA COMPATIBILITY
    const { error: updateError } = await supabase
      .from('users')
      .update({ 
        email_confirmed_at: new Date().toISOString(), // CHANGED: email_verified → email_confirmed_at
        email_verified: true 
      })
      .eq('email', email.toLowerCase());
...
app.post('/api/auth/login', authLimiter, async (req, res) => {
  try {
    ...
    // Verify password - UPDATED FOR SCHEMA COMPATIBILITY
    const validPassword = await bcrypt.compare(password, user.encrypted_password); // CHANGED: password_hash → encrypted_password
    if (!validPassword) {
      return res.status(401).json({ error: 'Invalid credentials' });
    }
    ...
    // Update last login - UPDATED FOR SCHEMA COMPATIBILITY
    await supabase
      .from('users')
      .update({ last_sign_in_at: new Date().toISOString() }) // CHANGED: last_login → last_sign_in_at
      .eq('id', user.id);
  ...
  try {
    ...
    const { error: tokenError } = await supabase
      ...
      .insert([
        {
          user_id: user.id,
          token_hash: resetToken, // CHANGED: token → token_hash to match schema
...
app.post('/api/auth/reset-password', authLimiter, async (req, res) => {
  try {
    ...
    // Check if token exists and is valid - UPDATED FOR SCHEMA COMPATIBILITY
    const { data: resetRecord, error: resetError } = await supabase
      .from('password_resets')
      .select('*')
      .eq('token_hash', token) // CHANGED: token → token_hash
      .gt('expires_at', new Date().toISOString())
      .single();
    ...
    // Update password - UPDATED FOR SCHEMA COMPATIBILITY
    const { error: updateError } = await supabase
      .from('users')
      .update({ encrypted_password: hashedPassword }) // CHANGED: password_hash → encrypted_password
      .eq('id', decoded.id);
    ...
    // Delete used reset token - UPDATED FOR SCHEMA COMPATIBILITY
    await supabase
      .from('password_resets')
      .delete()
      .eq('token_hash', token); // CHANGED: token → token_hash
