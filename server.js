...
// User Registration - ONLY SCHEMA FIXES
app.post('/api/auth/register', authLimiter, async (req, res) => {
  try {
    ...
    if (!allowedDomains.includes(domain)) {
      return res.status(400).json({ error: 'Please use Gmail, Outlook, Yahoo, or Hotmail email address' });
    }
    ...
    // Create user - SCHEMA FIX: encrypted_password instead of password_hash
      ...
      .insert([
        {
          ...
          encrypted_password: hashedPassword, // FIXED COLUMN NAME
          role: 'user',
          subscription_active: false,
          email_confirmed_at: null, // FIXED COLUMN NAME
          aud: 'authenticated', // REQUIRED
          instance_id: '00000000-0000-0000-0000-000000000000' // REQUIRED
        }
      ])
      .select()
      .single();
...
// Verify Email - ONLY SCHEMA FIXES
app.post('/api/auth/verify-email', authLimiter, async (req, res) => {
  try {
    ...
    // Mark email as verified - SCHEMA FIX: email_confirmed_at instead of email_verified
    const { error: updateError } = await supabase
      .from('users')
      .update({ 
        email_confirmed_at: new Date().toISOString(), // FIXED COLUMN NAME
        email_verified: true 
      })
      .eq('email', email.toLowerCase());
...
app.post('/api/auth/login', authLimiter, async (req, res) => {
  try {
    ...
    // Verify password - SCHEMA FIX: encrypted_password instead of password_hash
    ...
    // Update last login - SCHEMA FIX: last_sign_in_at instead of last_login
    await supabase
      .from('users')
...
// ALL YOUR ORIGINAL ENDPOINTS - COMPLETELY UNCHANGED:
...
// Reset Password - EXACTLY AS YOU HAD IT
app.post('/api/auth/reset-password', authLimiter, async (req, res) => {
  try {
    ...
    // Update password - SCHEMA FIX: encrypted_password instead of password_hash
    const { error: updateError } = await supabase
      .from('users')
...
// Activate Subscription (after payment) - EXACTLY AS YOU HAD IT
...
// Get User Profile - EXACTLY AS YOU HAD IT
...
// Protected sponsorship check - EXACTLY AS YOU HAD IT
...
// Your existing sponsorship check (public) - EXACTLY AS YOU HAD IT
...
// Helper functions (keep your existing ones) - EXACTLY AS YOU HAD IT
...
// Health check - EXACTLY AS YOU HAD IT
...
// Root endpoint - EXACTLY AS YOU HAD IT
app.get('/', (req, res) => {
  res.json({
...
// 404 handler - EXACTLY AS YOU HAD IT
app.use('*', (req, res) => {
  res.status(404).json({
...
// Error handler - EXACTLY AS YOU HAD IT
...
// Start server - EXACTLY AS YOU HAD IT
app.listen(PORT, () => {
